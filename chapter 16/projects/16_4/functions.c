...
void insert()
{
   ...
   printf("Enter price of the new item in dollar: ");
   fflush(stdout);
   scanf("%f",( (&item[current_used_items].item_price) ) );
		...
		for ( counter=0; counter<=current_used_items; counter++)
				{
					...
					printf("		item price    : %.2f$",item[counter].item_price);
 ...
 if( ( current_used_items > EMPTY ) &&  (current_used_items<struct_max_elements) ) // if Inventory not empty
	{
			...
			if(index != EMPTY)
			{
	             ...
	             if( (item[index].quantity + data_input) < 0 ) printf("input not valid! Entered quantity is more than existed quantity\n");
	             else  item[index].quantity+=data_input;

	             printf("Enter change in item price on hand");
	             fflush(stdout);
	             scanf("%f",&change_in_price);
	             if( (item[index].item_price + change_in_price) < 0 ) printf("invalid operation!! .. cut in price is more than existed price\n");
	             else  item[index].item_price+=change_in_price;
